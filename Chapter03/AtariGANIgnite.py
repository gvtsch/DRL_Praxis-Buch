...
# dim input image will be rescaled
IMAGE_SIZE = 64
...
class Discriminator(nn.Module):
    def __init__(self, input_shape):
        ...
        # this pipe converges img into the single number
...
def iterate_batches(envs, batch_size=BATCH_SIZE):
    ...
    while True:
        ...
        if len(batch) == batch_size:
            # normalize input
            batch_np = np.array(batch, dtype=np.float32) * 2.0 / 255.0 - 1
...
if __name__ == "__main__":
    ...
    print(true_labels_v, fake_labels_v)
